 ...
 *  1. Creates the Supabase auth user (email confirmed immediately for MVP).
 *  2. Creates the firm row + owner membership using the service-role client
 *     (RLS would otherwise block the very first insert, before a firm exists).
 *
 * The client then signs in with the same credentials.
 */
